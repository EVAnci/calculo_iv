...
### Revisión ortográfica, gramática y de escritura

Para verificar la ortografía del texto escrito en LaTeX recomiendo usar [textidote](https://github.com/sylvainhalle/textidote). Para comprobar la ortografía de todo el documento puede usar 
```sh
textidote --check es --output html *.tex > report.html
firefox report.html
```
Y para un único archivo
```sh
textidote --check es --output html archivo.tex > report.html
firefox report.html
```
Luego, en firefox (o el navegador que más le guste) puede revisar las sugerencias del reporte. El reporte nunca se debe agregar al repositorio, en el archivo `.gitignore` está contemplado ingnorar a `report.html`, pero si usa otro nombre debe cuidar no añadirlo.
