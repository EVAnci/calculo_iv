\chapter{Ejercicios Recomendados}

Esta sección contiene una serie de ejercicios generales, recomendados de cada capítulo. Hacer estos ejercicios puede servir para terminar de afianzar los conocimientos antes de rendir un examen.

Cada ejercicio se colocará con la respuesta, pero sin el procedimiento.

\section{Funciones de variable compleja}

\begin{exercise}
  Verificar la siguiente identidad:
  $$
  (\cos(\theta)+j\sin(\theta))^n = \cos(n\theta) + j\sin(n\theta)
  $$
  \textit{Solución}: Si se verifica, es llamada fórmula de Moivre.
\end{exercise}

\begin{exercise}
  Expresar las siguientes funciones de variable compleja en la forma $f(z)=u(x,y)+jv(x,y)$.
  \begin{enumerate}
    \item Para $f(z)=3z^{-2}+2z$. 

      \textit{Solución}:
      $$
      u(x,y) = \frac{3(x^2-y^2)}{(x^2+y^2)}+2x \qquad v(x,y)=2y-\frac{6xy}{(x^2+y^2)^2}
      $$
    \item Para $f(z)=\sin(2z)$.

      \textit{Solución}:
      $$
      u(x,y) = \sin(2x)\,\cosh(2y) \qquad v(x,y)=\cos(2x)\,\senoh(2y)
      $$
    \item Para $f(z)=\ln(z)$.

      \textit{Solución}:
      $$
      u(x,y) = \frac{1}{2}\ln(x^2+y^2) \qquad v(x,y)=\arctan\left(\frac{y}{x}\right)
      $$
  \end{enumerate}
  \label{ej:expresar_funciones_como_u_y_v}
\end{exercise}

\begin{exercise}
  Comprobar si las funciones del ejercicio \ref{ej:expresar_funciones_como_u_y_v} cumplen las ecuaciones de Cauchy-Riemann.

  \textit{Soluciones}:
  \begin{enumerate}
    \item Para $f(z)=3z^{-2}+2z$ las derivadas de $u$ y $v$ son:
      \begin{gather*}
        u_x = \frac{-6x^3+18y^2}{(x^2+y^2)^3}+2 \qquad v_y = u_x \\ 
        u_y = -\frac{18x^2y-6y^3}{(x^2+y^2)^3} \qquad v_x = -u_y
      \end{gather*}
      Por tanto, si cumple con las ecuaciones de Cauchy-Riemann.
    
    \item Para $f(z)=\sin(2z)$ las derivadas de $u$ y $v$ son:
      \begin{gather*}
        u_x = 2\cos(2x)\,\cosh(2y) \qquad v_y = u_x \\ 
        u_y = 2\sin(2x)\,\senoh(2y) \qquad v_x = -u_y
      \end{gather*}
      Por tanto, si cumple con las ecuaciones de Cauchy-Riemann.

    \item Para $f(z)=\ln(z)$ las derivadas de $u$ y $v$ son:
      \begin{gather*}
        u_x = \frac{x}{x^2+y^2} \qquad v_y = u_x \\ 
        y_y = \frac{y}{x^2+y^2} \qquad v_x = -u_y
      \end{gather*}
      Por tanto, si cumple con las ecuaciones de Cauchy-Riemann.
  \end{enumerate}
\end{exercise}

\begin{exercise}
  Hallar la función analítica $f(z)$ a partir de su parte real $u(x,y)=2e^x\cos(y)$ con la condición $f(0)=7$.

  \textit{Solución}: Integrando sobre las ecuaciones de Cauchy-Riemann obtenemos
  $$
  v(x,y) = 2e^x\sin(y) + C
  $$
  Para cumplir la condición $f(0)=7$ reemplazamos sobre $f(z)=2e^x\cos(y) + j (2e^x\sin(y) + C)$, para determinar $C$, resultando 
  $$
  C=-5j \quad \Rightarrow \quad f(z)=2e^x\cos(y) + j2e^x\sin(y) + 5
  $$
  Escrito en notación compacta:
  $$
  f(z)=2e^z + 5
  $$
\end{exercise}

\begin{exercise}
  Comprobar la función $u(x,y)=x^3-3xy^2$ es armónica y, en caso de ser afirmativo, calcular su armónica conjugada.

  \textit{Solución}: Si es armónica. Su armónica conjugada es 
  $$
  v(x,y)=3x^2y-y^3 +C
  $$
  Y, tomando una solución particular con $C=0$, componen la función:
  $$
  f(z)=x^3-3xy^2+j3x^2y - jy^3 \qquad \Rightarrow \qquad f(z)=z^3
  $$
\end{exercise}

\section{Integración Compleja}

\begin{exercise}
  Calcular las siguientes integrales utilizando las fórmulas de Cauchy.
  \begin{enumerate}
    \item Teniendo la curva $\Gamma$ dada por: 
      \begin{itemize}
        \item $\lvert z-1 \rvert =4$
        \item $\lvert z-2\rvert + \lvert z+2\rvert = 6$
      \end{itemize}
      Resolver: 
      $$
      \oint_\Gamma \frac{e^{3z}}{z-j\pi}dz
      $$
    \item Teniendo en cuenta $\Gamma$ dada por $\lvert z-1 \rvert =2$ resolver 
      $$
      \oint_\Gamma \frac{z}{2z-5}dz
      $$
  \end{enumerate}
  \textit{Soluciones}:
  \begin{enumerate}
    \item Para $\lvert z-1 \rvert =4$ tenemos 
      \begin{gather*}
        \text{Si } z = j\pi \quad \Rightarrow \quad \lvert j\pi - 1 \rvert < 4  
      \end{gather*}
      Entonces la singularidad del integrando queda encerrada en $\Gamma$. Usamos la fórmula de la integral de Cauchy:
      $$
      \oint_\Gamma \frac{e^{3z}}{z-j\pi}dz = j2\pi e^{-3j\pi} = \boxed{j\frac{2\pi}{e^{j3\pi}}}
      $$
      Para $\lvert z-2\rvert + \lvert z+2\rvert = 6$ representa una elipse como se muestra en la figura \ref{fig:ej:integral_elipse} tenemos que: 
      \begin{gather*}
        \text{Si }z=j\pi \quad \Rightarrow \quad \lvert j\pi-2\rvert + \lvert j\pi+2\rvert \approx 7.45 > 6 
      \end{gather*}
      \begin{figure}[ht]
        \centering
        \begin{tikzpicture}[scale=0.75,>=stealth]
          \draw[->,thin] (-4,0) -- (4,0) node[right] {$\Re(z)$};
          \draw[->,thin] (0,-3.5) -- (0,3.5) node[right] {$\Im(z)$};
          \begin{scope}[decoration={
              markings,
              mark=at position 0.1 with {\node[above right] {$\Gamma$};},
            }]
            \draw[very thick,red,postaction={decorate}] (0,0) ellipse (3 and 2.236);
          \end{scope}
          \foreach \x in {-3,-2,2,3} {
            \draw[thin] (\x,1pt) -- (\x,-1pt) node[below left] {\scriptsize$\x$};
          }
          \foreach \y\r in {-3.14/$-\pi$,-2.236/$-\sqrt{5}$,2.236/$\sqrt{5}$} {
            \draw[thin] (-1pt,\y) -- (1pt,\y) node[above right] {\scriptsize\r};
          }
          \draw[thick,fill=white] (0,-3.14) circle (2pt);
        \end{tikzpicture}
        \caption{Figura de la curva $\Gamma$ del ejercicio \ref{ej:integral_elipse}.}
        \label{fig:ej:integral_elipse}
      \end{figure}
      Por tanto queda fuera de la curva $\Gamma$ y 
      $$
      \oint_\Gamma \frac{e^{3z}}{z-j\pi}dz = 0
      $$
    \item Primero operamos sobre el denominador del integrando:
      $$
      2z-5 = 0 \quad\Rightarrow\quad z = \frac{5}{2}
      $$
      Para $\lvert z-1 \rvert =2$ tenemos que
      \begin{gather*}
        \text{Si } z=\frac{5}{2} \quad \Rightarrow \quad \lvert 5/2 -1 \rvert < 2
      \end{gather*}
      Por tanto está dentro de $\Gamma$. Usamos la fórmula de la integral de Cauchy:
      $$
      \oint_\Gamma \frac{z}{2z-5}dz = j5\pi 
      $$
  \end{enumerate}
  \label{ej:integral_elipse}
\end{exercise}

\section{Teorema de los Residuos}

\begin{exercise}
  Hallar la serie de Laurent alrededor de las siguientes singularidades indicadas para cada una de las siguientes funciones. 
  \begin{enumerate}
    \item $f(z)=\frac{z}{(z+1)(z+2)}$ alrededor de $z=-1$ y luego alrededor de $z=-2$.
    \item $f(z)=\frac{e^{3z}}{(z-1)^3}$ alrededor de $z=1$.
    \item $f(z)=\frac{\cos(2z)}{z^2}$ alrededor de $z=0$.
  \end{enumerate}
  \textit{Soluciones}: 
  \begin{enumerate}
    \item Serie de Laurent desarrollada con centro en $z=-1$:
      $$
      f(z)=-\frac{1}{z+1} + 2\cdot\sum_{n=0}^\infty (-1)^{n}(z+1)^{n}
      $$
      Donde el radio de convergencia (o corona donde la serie es válida) es $0<\lvert z+1 \lvert < 1$.

      Luego, la misma función desarrollada en serie de Laurent con centro en $z=-2$:
      $$
      f(z)=\frac{2}{z+2} + \sum_{n=0}^\infty (z+2)^n
      $$
      válida en la corona $0<\lvert z+2\rvert <1$.
    \item La serie de Laurent con centro en $z=1$:
      $$
      f(z)=e^3\cdot\sum_{n=0}^\infty \frac{3^n}{n!}(z-1)^{n-3}
      $$
      en la corona $0<\lvert z-1 \rvert <\infty$.
    \item La serie de Laurent con centro en $z=0$ es:
      $$
      f(z)=\sum_{n=0}^\infty (-1)^n\frac{4^n}{(2n)!}z^{2n-2}
      $$
      válida en la corona $0<\lvert z \rvert < \infty$.
  \end{enumerate}
\end{exercise}

\begin{exercise}
  Calcular el residuo de la siguiente función en sus polos.
  $$
  f(z)=\frac{z^2-2z}{(z+1)^2(z^2+4)}
  $$
  \textit{Solución}: reescribimos la función factorizando:
  $$
  f(z)=\frac{z^2-2z}{(z+1)^2(z+2j)(z-2j)}
  $$
  Calculamos el residuo para cada uno de los polos.
  \begin{itemize}
    \item Para $z=-1$ hay un polo múltiple. El residuo es 
      $$
      \res_{z=-1}f(z)=\frac{1}{1!}\lim_{z\to-1}\left(\frac{d}{dz}\left(\frac{z^2-2z}{z^2+4}\right)\right) = -\frac{14}{25}
      $$
    \item Para $z=2j$ hay un polo simple. El residuo es 
      $$
      \res_{z=2j}f(z)=\lim_{z\to2j} \left(\frac{z^2-2z}{(z+1)^2(z+2j)}\right) = \frac{7}{25} + \frac{1}{25}j
      $$
    \item Para $z=-2j$ hay un polo simple. El residuo es 
      $$
      \res_{z=-2j}f(z)=\lim_{z\to-2j}\left( \frac{z^2-2z}{(z+1)^2(z-2j)} \right) = \frac{7}{25}-\frac{1}{25}j
      $$
  \end{itemize}
\end{exercise}

\section{Integración de funciones reales}

\begin{exercise}
  Resolver la siguiente integral real utilizando los métodos de resolución de variable compleja.
  $$
  I = \int_{-\infty}^\infty \frac{x\sin(2x)}{x^2+4x+20}dx
  $$
  \textit{Solución}: Primero analizamos el integrando. Es de la forma $Q(x)\sin(mx)$, donde $Q$ no tiene raíces reales, sino complejas conjugadas. Además $Q$ tiende uniformemente a cero cuando $x\to \infty$. Entonces llamamos al polo $z_0=-2+4j$ que está en el semiplano superior.

  Se utiliza la solución vista en la sección \ref{sec:racional_trigonometrica_analitica_en_r}, entonces 
  $$
  \int_{-\infty}^\infty \frac{x\sin(2x)}{x^2+4x+20}dx = \Im\left( j2\pi\cdot\res_{z=z_0}Q(z)e^{j2z} \right) 
  $$
  dando como resultado 
  $$
  \int_{-\infty}^\infty \frac{x\sin(2x)}{x^2+4x+20}dx = \frac{\pi}{2e^8}(\sin(4)+2\cos(4))
  $$
  quedando resuelta la integral.
\end{exercise}

\begin{exercise}
  Resolver utilizando residuos.
  $$
  \int_{-\infty}^\infty \frac{x^3}{(1+x^2)^3}dx
  $$
  \textit{Solución}: Analizamos la integral: es del tipo $\int_{-\infty}^\infty p(x)/q(x)dx$. Vemos que $q$ es tres grados mayor que $p$, entonces podemos asegurar convergencia. El polo en el semiplano superior es $z_0=j$. Usando el resultado de la sección \ref{sec:racionales_analiticas},
  $$
  \oint_\Gamma \frac{z^3}{(1+z^2)^3}dz = j2\pi \cdot \res_{z=i} \frac{z^3}{(1+z^2)^3} = \int_{-\infty}^\infty \frac{x^3}{(1+x^2)^3}dx
  $$
  Aquí $z_0$ es un polo múltiple de orden tres, por tanto el residuo es 
  $$
  \res_{z=i} \frac{z^3}{(1+z^2)^3} =\frac{1}{2!} \lim_{z\to j} \frac{d^2}{dz^2}\left(\frac{z^3}{(z+j)^3}\right) = 0
  $$
  Resultando entonces la integral 
  $$
  \int_{-\infty}^\infty \frac{x^3}{(1+x^2)^3}dx = 0
  $$
  Lo cual tiene sentido por ser el integrando una función impar.
\end{exercise}

\begin{exercise}
  Demostrar que 
  $$
  \int_0^{2\pi} \frac{1}{(2+\cos(\theta))^2}d\theta = \frac{4\pi\sqrt{3}}{9}
  $$
  \textit{Solución}: Usando el resultado de \ref{sec:integrales_reales_trigonometricas}, llamamos $f(z)$ a 
  $$
  f(z)=\frac{1}{jz\left(2+(\frac{z+\bar{z}}{2})\right)} = \frac{4z}{j((z+2+\sqrt{3})(z+2-\sqrt{3}))^2}
  $$
  Se tiene solo un polo de orden dos dentro de la circunferencia unitaria. Llamamos $z_0=-2+\sqrt{3}$. Entonces
  \begin{align*}
    \frac{4\pi\sqrt{3}}{9} &= j2\pi \res_{z=z_0}f(z) \\ 
                           &=j2\pi\lim_{z\to z_0}\frac{d}{dz}\left(\frac{4z}{j(z+2+\sqrt{3})^2}\right) \\ 
                           &= 8\pi \lim_{z\to z_0}\frac{2+\sqrt{3}-z}{(z+2+\sqrt{3})^3} \\ 
    \frac{4\pi\sqrt{3}}{9} &= \frac{4\pi\sqrt{3}}{9}
  \end{align*}
  quedando demostrada la identidad.
\end{exercise}
