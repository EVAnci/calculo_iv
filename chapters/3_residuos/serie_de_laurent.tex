...
donde 
$$
c_k = \frac{1}{j2\pi}\oint_\Gamma \frac{f(z^\ast)}{(z^\ast-z_0)^{k+1}}\,dz \qquad b_k = \frac{1}{j2\pi}\oint_\Gamma \frac{f(z^\ast)}{(z^\ast-z_0)^{-k+1}}\,dz
$$
Entonces, vemos que tanto el coeficiente $b_k$ como $(z^\ast-z_0)$ usando expresiones como las de la serie de Taylor solo cambian su signo. Bajo este punto de vista, como el primer término recorre el intervalo $[0,\infty)$ y el segundo término recorre $(-\infty,1]$ podemos escribir la serie de forma compacta como 
\begin{equation}
...
$$
c_k=\frac{1}{j2\pi}\oint_\Gamma \frac{f(z^\ast)}{(z-z_0)^{k+1}}\,dz
$$
...
\begin{example}[Uso de la serie de Maclaurin]
  ...
  $$
  z^{-5}\sin(z) = \sum{n=0}^\infty \frac{(-1)^n}{(2n+1)!} \frac{z^{2n+1}}{z^5} = \boxed{\sum{n=0}^\infty \frac{(-1)^n}{(2n+1)!} z^{2n-4}}
  $$
...
\begin{example}[Sustitución]
  ...
  Entonces 
  $$
  e^{1/z} = \sum{n=0}^\infty \frac{1}{n!}\cdot\left(\frac{1}{z}\right)^n = \sum_{n=0}^\infty \frac{1}{n!\,z^n}
  $$
...
\begin{example}[Desarrollo de series en coronas concéntricas diferentes]
  ...
  $$
  Por tanto, podemos desarrollar por serie geométrica $1/(1-z)$, siendo 
  $$
  \frac{1}{z^3-z^4} = \frac{1}{z^3}\sum_{n=0}^\infty z^n = \boxed{\sum_{n=0}^\infty z^{n-3}}
  $$
  donde $z$ es válida en la corona $0<\lvert z \rvert < 1$. 

  Sin embargo, también podemos desarrollar la misma $1/(1-z)$ en potencias negativas (que será válida en otra corona).
  $$
  ...
  $$
  \frac{1}{z^3-z^4}=\sum_{n=0}^\infty \frac{1}{z^{n+1}}\cdot \frac{1}{z^3} = \boxed{\sum_{n=0}^\infty \frac{1}{z^{n+4}}}
  $$
  donde $z$ es válida en la corona $\lvert z \rvert >1$.
\end{example}
...
\begin{definition}[Aislada]
  Una función compleja $f$ tiene una singularidad aislada en $z_0$ si $f$ es diferenciable en un anillo $0<\lvert z-z_0\rvert < R$ (la distancia al punto $z_0$ es mayor que cero y menor que $R$). 
\end{definition}
Por ejemplo, $1/z$ tiene una singularidad aislada en $0$, y $\sin(z)/(z-\pi)$ tiene una singularidad aislada en $\pi$.
...
\begin{definition}[Removible]
  ...
  Este tipo de singularidades son de poco interés ya que la singularidad, como su nombre lo indica, pueden quitarse.
\end{definition}
...
\begin{example}[Algunos ejemplos de singularidades]
  ...
  Por otro lado, un ejemplo de polo múltiple es $f(z)=a/z^4$ que tiene un polo múltiple de orden $4$ en $z=0$.
\end{example}
