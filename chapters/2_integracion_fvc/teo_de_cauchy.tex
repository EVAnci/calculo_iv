...
\input{chapters/2_integracion_fvc/media/trayectorias_cerradas.tex}
...
\begin{definition}[Dominio simplemente conexo]
  Como se vio en la sección \ref{sec:1_curvas_y_regiones_en_el_plano_complejo} un dominio es un conjunto conexo abierto. Ahora, decimos que un dominio $D$ es simplemente conexo si además de cumplir con la definición de dominio cumple con la definición \ref{def:simplemente_conexo}.
\end{definition}

Por ejemplo, en la figura \ref{fig:doblemente_conexo} si se traza una trayectoria circular $\Gamma$ dentro de la región pintada, esta encierra puntos que no pertenecen a $D$ (que son los puntos dentro de la elipse pequeña que delimita el interior de la región). Lo mismo sucede con la región de la figura \ref{fig:triplemente_conexo}.
...
\input{chapters/2_integracion_fvc/media/conexidad_dominios.tex}

Ahora sí, estamos listos para enunciar el teorema de Cauchy.
...
Este teorema, solo se cumple si la función es analítica. Si $f(z)$ no es analítica nada podemos asegurar con respecto a su resultado, ya que puede o no ser cero. Veamos un ejemplo de cada una.
...
\section[T. de Cauchy para dominios múltiplemente conexos]{El teorema de Cauchy para dominios \\ múltiplemente conexos}
...
Al intentar aplicar el teorema de Cauchy usual debemos verificar que $f$ sea analítica. Entonces verificamos las ecuaciones de Cauchy-Riemman:
$$
...
$$
¿Y qué pasaría si aumentamos el tamaño de la curva $\Gamma$ o damos una curva cualquiera $\Phi$ que encierre el punto $z_0$ pero sea difícil de parametrizar? 
\end{example}
...
\subsection{Demostración del primer colorario}
...
\begin{tikzpicture}[>=stealth]
  ...
  \begin{scope}[thick,red,decoration={
    ...
    \draw[postaction={decorate}] ({cos(30)},{sin(30)}) -- ({2*cos(30)},{2*sin(30)}) node[right] {$\color{black}\gamma_1$};
    \draw[postaction={decorate}] ({cos(200)},{sin(200)}) -- ({2*cos(200)},{2*sin(200)}) node[left] {$\color{black}\gamma_2$};
  \end{scope}
...
Llamaremos $\Gamma_1$ a la mitad superior de la curva $C_1$, $\Gamma_2$ a la mitad superior de la curva $C_2$, $\Gamma_3$ a la mitad inferior de $C_2$ y $\Gamma_4$ a la mitad inferior de $C_1$.

Entonces la concatenación $\Phi_1 = \gamma_1 \oplus \Gamma_1 \oplus \gamma_2 \oplus \Gamma_2$ es la \textbf{curva que encierra a} $D_1$. Y $\Phi_2 = \gamma_1 \oplus \Gamma_3 \oplus \gamma_2 \oplus \Gamma_4$ es la \textbf{curva encierra al dominio} $D_2$. Aquí el símbolo $\oplus$ se usa para indicar la \textit{concatenación} de curvas.

\begin{proof}
  Mediante los cortes $\gamma_1$ y $\gamma_2$, $D$ se divide en dos dominios simplemente conexos $D_1$ y $D_2$, en los cuales y sobre los cuales $f(z)$ es analítica. Por el teorema de Cauchy, la integral sobre la curva $\Phi_1$ que encierra al dominio de arriba $D_1$ es cero, y la integral sobre la curva $\Phi_2$ que encierra el dominio $D_2$ también es cero. Entonces
  $$
  \oint_{\Phi_1} f(z)dz + \oint_{\Phi_2} f(z)dz = 0
  $$
  la suma de las integrales es cero. Aquí, si vemos el sentido de circulación sobre los segmentos $\gamma_1$ y $\gamma_2$ (los cortes) vemos que cada integral los recorre en un sentido, por tanto al sumar las integrales descomponiendo la concatenación, se cancelarán. Véase, para el primer término del miembro izquierdo (curva $\Phi_1$)
  $$
    \int_{\gamma_1} f(z)dz + \int_{\Gamma_1} f(z)dz + \int_{\gamma_2} f(z)dz + \int_{\Gamma_2} f(z)dz 
  $$
  y para el segundo término (curva $\Phi_2$)
  $$
    -\int_{\gamma_1} f(z)dz + \int_{\Gamma_3} f(z)dz - \int_{\gamma_2} f(z)dz + \int_{\Gamma_4} f(z)dz 
  $$
  Como bien se ve, por ser $\gamma_1$ y $\gamma_2$ curvas paramétricas, cambiar el signo implica recorrerla en sentido opuesto (ver propiedades de las integrales sección: \ref{sec:propiedades_de_las_integrales}), entonces, al sumar estos dos resultados, se cancelaran, y el resultado es 
  $$
    \int_{\Gamma_1} f(z)dz + \int_{\Gamma_2} f(z)dz + \int_{\Gamma_3}f(z)dz + \int_{\Gamma_4} f(z)dz = 0
  $$
  Pero, $\Gamma_1\oplus \Gamma_4 = C_1$ y $\Gamma_2\oplus\Gamma_3 = C_2$, por tanto
  $$
    \ointctrclockwise_{C_1} f(z)dz + \ointclockwise_{C_2} f(z)dz = 0
  $$
  Al invertir la trayectoria $C_2$ resulta 
  $$
    \ointctrclockwise_{C_1} f(z)dz - \ointctrclockwise_{C_2} f(z)dz = 0
  $$
  Que no es más que la expresión del teorema \ref{teo:cauchy_doblemente_conexo}, véase
  $$
  \oint_{C_1}f(z)dz = \oint_{C_2}f(z)dz 
  $$
...
\subsection{Segundo colorario de Cauchy}

Para dominios de conexidad superior la idea es la misma. Así, para un dominio triplemente conexo se usan tres cortes $\gamma_1$, $\gamma_2$ y $\gamma_3$. Al sumar las integrales como antes, las integrales sobre los cortes se cancelan y la suma de las integrales sobre $C_1$ (sentido contrario a las manecillas del reloj) y $C_2$, $C_3$ (en sentido del movimiento de las manecillas del reloj) es cero. Así la integral sobre $C_1$ es igual a la suma de las integrales sobre $C_2$ y $C_3$, en donde ahora las tres se consideran en sentido contrario a las manecillas del reloj.
\begin{equation}
\oint_{C_1} f(z)dz = \oint_{C_2} f(z)dz + \oint_{C_3} f(z)dz
\label{eq:dom_de_conex_sup}
\end{equation}
