...
\begin{proof}
  ...
  \end{equation}
  Resolviendo el primer término del miembro derecho, vemos que $\Gamma$ es una curva paramétrica no conocida que encierra al punto $z_0$. Con ello, podemos aplicar el teorema de deformación \ref{teo:cauchy_doblemente_conexo} y podemos sustituir la curva $\Gamma$ por una circunferencia unitaria $\gamma$ que encierre a $z_0$
  \begin{gather*}
  ...
  \begin{align*}
    \int_0^{2\pi} \frac{f(\gamma(t))-f(z_0)}{e^{jt}} ~ je^{jt} ~dt &= j\int_0^{2\pi} f(\gamma(t))-f(z_0) dt \\
                                                                   &= j\left( \int_0^{2\pi} f(z_0+e^{jt}) dt - f(z_0)\int_0^{2\pi}1 dt \right)
  \end{align*}
